...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = @selector(cocoanidaq_nextEventMatchingMask:untilDate:inMode:dequeue:);
...
- (NSEvent *)cocoanidaq_nextEventMatchingMask:(NSEventMask)mask
                                    untilDate:(NSDate *)expiration
                                       inMode:(NSRunLoopMode)mode
                                      dequeue:(BOOL)deqFlag
{
    if (NSThread.isMainThread) {
        return [self cocoanidaq_nextEventMatchingMask:mask untilDate:expiration inMode:mode dequeue:deqFlag];
    }
    
    __block NSEvent *event = nil;
    dispatch_sync(dispatch_get_main_queue(), ^{
        event = [[self cocoanidaq_nextEventMatchingMask:mask untilDate:expiration inMode:mode dequeue:deqFlag] retain];
    });
